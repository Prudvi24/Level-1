...
public:
  ...
  void oddEven(){
    Node *temp = this->head;
    Node *odd=nullptr, *oddptr=nullptr;
    Node *even=nullptr, *evenptr=nullptr;
    while(temp){
      Node *nn = new Node(0);
      if(temp->data%2){
        nn->data = temp->data;
...
linkedlist mergeSort(Node* head, Node* tail ) {
  ...
  linkedlist left_ll = mergeSort(head, midnode);
  linkedlist right_ll = mergeSort(midnode->next, tail);
  res = mergeTwoSortedLists(left_ll, right_ll);
  return res;
}
